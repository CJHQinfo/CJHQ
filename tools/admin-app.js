...
async function removeStaffMember(emailRaw){
  const email = staffDocId(emailRaw);
  if(await staffIsBuiltIn(email)){
    alert('This is a built-in account. Removing it here would change nothing, because the ' +
  ...
  if(adminUser && staffDocId(adminUser.email) === email){
    alert('You cannot remove your own access from here. Ask another staff member to do it.');
    return false;
  }
  if(!firebaseReady || !window.__fbFirestore){
    alert('Not connected to the database. Nothing was changed.');
    return false;
  }
...
async function backfillCustomPageLifecycle(){
  ...
  if(!confirm('Update ' + stale.length + ' existing page(s) so they work with start and end dates?\n\n' +
  ...
  await enhancePagesFromBackend();
  renderCustomPagesList();
  alert('Updated ' + stale.length + ' page(s).');
}
...
async function savePartnerContact(key){
  ...
  if(existing.contactVerified === true && changed){
    ...
    alert('Contact details changed, so verification was reset. Please re-confirm against the organization, then tick Contact verified again.');
  }
...
(function initAdmin(){
  ...
  [['En','customPageBodyEn'], ['Fr','customPageBodyFr']].forEach(([side, target])=>{
    ...
    input.addEventListener('change', ()=>{
      ...
      if(file.size > 512 * 1024){
        alert('That file is larger than 512 KB. Page bodies are stored in the database and downloaded by every visitor, so please trim it first.');
        input.value = ''; return;
      }
  ...
  document.getElementById('customPageForm').addEventListener('submit', async (e)=>{
    ...
    if(page.start_date && page.end_date && page.end_date < page.start_date){
      alert('The End Date is before the Start Date, so the page would never be public. Please correct the dates.');
      return;
    }
...
(function initVisibilityAdmin(){
    ...
    SECTIONS.forEach(sec=>{
      ...
      saveOnly.addEventListener('click', async ()=>{
        if(pending.startsOn && pending.endsOn && pending.endsOn < pending.startsOn){
          alert('The end date cannot be earlier than the start date.'); return;
        }
        saveOnly.disabled = true;
        try{
          await setSectionVisibility(sec.key, published, adminUser && adminUser.email, pending);
          render();
        }catch(err){ alert('Could not save the dates.'); }
      ...
      toggle.addEventListener('click', async ()=>{
        ...
        try{
          ...
          alert('Could not change visibility. Check that you are still signed in and that the Firestore rules allow writing to settings.');
  ...
  async function persist(notice, changes, btn){
    ...
    try{
      ...
      alert('Could not save. Check that you are still signed in.');
...
(function initAskCjhq(){
  ...
  $a('askCopy').addEventListener('click', ()=>{
    if(!lastAnswer){ alert('No response to copy yet.'); return; }
  ...
  function renderSources(){
    ...
    askSources.forEach((s, i)=>{
      ...
      d.addEventListener('click', ()=>{ if(confirm('Remove "' + s.name + '" from the registry?')) removeSource(s); });
  ...
  async function saveSource(src){
    try{
      ...
      alert('Could not save. The ask_sources collection may not be permitted by the published Firestore rules yet.');
    }
  }
  ...
  async function removeSource(src){
    try{
      ...
      alert('Could not remove. Check the Firestore rules for ask_sources.');
    }
  }

  $a('askSeedSources').addEventListener('click', async ()=>{
    if(!confirm('Load the default source registry? Existing entries with the same id will be overwritten.')) return;
    for(const s of ASK_DEFAULT_SOURCES){ await saveSource(s); }
  });

  $a('askAddSource').addEventListener('click', async ()=>{
    const name = prompt('Source name:'); if(!name) return;
    const url  = prompt('Source URL (https://…):') || '';
    if(url && !cjhqSafeUrl(url)){ alert('That URL is not a valid http(s) address.'); return; }
    const type = prompt('Source type (' + ASK_SOURCE_TYPES.join(', ') + '):', 'Community') || 'Other';
    const src = {
      ...
      active:true, authoritative: confirm('Mark as authoritative?'),
      timeSensitive: confirm('Is this information time-sensitive?'),
  ...
  if(_bCopy) _bCopy.addEventListener('click', ()=>{
    if(!__benchRows.length){ alert('Run the review first.'); return; }
    ...
    if(navigator.clipboard && navigator.clipboard.writeText){
      navigator.clipboard.writeText(header + txt)
        .then(()=> alert('Copied. Paste it into an email for the reviewer.'))
