 ...
 * ================================================================ */
const AI_CHECK_URL = 'https://instructioncheck-qu3jib66wa-ue.a.run.app/?go=1'; // trailing query: the bare URL's CORS preflight intermittently fails in some browsers (redirect-class fetch failure); a query string avoids it
...
// "20260924" -> 2026-09-24, "2026092414" -> 2026-09-24 14:00
function anaFmtBucket(k){
  k = String(k || '');
  const d = k.slice(0,4) + '-' + k.slice(4,6) + '-' + k.slice(6,8);
  return k.length >= 10 ? d + ' ' + k.slice(8,10) + ':00' : d;
}

/* ---------- Export: print / PDF, spreadsheet, share ---------- */
function anaCsv(){
  const d = ANA_LAST; if(!d || !d.ga) return '';
  const g = d.ga, t = g.totals || {};
  const q = v => { v = v == null ? '' : String(v); return /[",\n]/.test(v) ? '"' + v.replace(/"/g, '""') + '"' : v; };
  const out = [];
  const line = (...a) => out.push(a.map(q).join(','));
  const block = (title, head, list, f) => { out.push(''); line(title); line(...head); (list || []).forEach(x => line(...f(x))); };
  line('CJHQ website analytics', g.label || '');
  line('Generated', new Date(d.generatedAt).toLocaleString('en-CA', { timeZone:'America/Toronto' }));
  out.push('');
  line('Totals');
  [['Visitors', t.visitors], ['Visits', t.visits], ['Page views', t.pageViews], ['Time on site per visitor (seconds)', t.avgEngagedSecondsPerVisitor],
   ['Engaged visits (%)', Math.round((t.engagementRate || 0) * 100)], ['New visitors', t.newVisitors], ['Events', t.events]].forEach(r => line(...r));
  block(g.hourly ? 'Visitors per hour' : 'Visitors per day', [g.hourly ? 'Hour' : 'Date', 'Visitors', 'Page views'], g.daily, x => [anaFmtBucket(x.date), x.visitors, x.pageViews]);
  block('Pages', ['Page', 'Views', 'Visitors', 'Avg. seconds on page'], g.pages, x => [x.path, x.views, x.visitors, x.avgSecondsOnPage]);
  block('Visits by tracking link', ['Name / platform', 'Page', 'Visits', 'Visitors'], g.tracked, x => [x.label, x.page, x.visits, x.visitors]);
  const dm = d.demographics || {};
  block('Top cities', ['Place', 'Visitors'], dm.cities, x => [x.place, x.visitors]);
  block('Top countries', ['Country', 'Visitors'], dm.countries, x => [x.country, x.visitors]);
  block('What visitors did', ['Action', 'Count'], g.events, x => [ANALYTICS_EVENT_LABELS[x.name] || x.name, x.count]);
  block('Links clicked to other sites', ['Link', 'Clicks'], g.links, x => [x.url, x.clicks]);
  block('Where visitors came from', ['Source / medium', 'Visits'], g.sources, x => [x.source, x.visits]);
  block('Devices', ['Device', 'Visitors'], g.devices, x => [x.device, x.visitors]);
  return '\ufeff' + out.join('\r\n');
}
function anaFileName(){
  const g = (ANA_LAST && ANA_LAST.ga) || {};
  const day = new Date().toLocaleDateString('en-CA', { timeZone:'America/Toronto' });
  return 'cjhq-analytics-' + String(g.range || 'report').replace(/[^a-z0-9_-]/gi, '') + '-' + day + '.csv';
}
function anaDownload(name, text){
  const url = URL.createObjectURL(new Blob([text], { type:'text/csv;charset=utf-8' }));
  const a = document.createElement('a'); a.href = url; a.download = name;
  document.body.appendChild(a); a.click(); a.remove();
  setTimeout(() => URL.revokeObjectURL(url), 4000);
}
function anaSummaryText(){
  const g = (ANA_LAST && ANA_LAST.ga) || {}, t = g.totals || {};
  const top = (g.pages || []).slice(0, 3).map(p => p.path + ' (' + p.views + ')').join(', ');
  return 'CJHQ website - ' + (g.label || '') + ': ' + anaFmtNum(t.visitors) + ' visitors, ' + anaFmtNum(t.visits) + ' visits, '
    + anaFmtNum(t.pageViews) + ' page views.' + (top ? ' Top pages: ' + top + '.' : '');
}
function anaWireExports(){
  const msg = document.getElementById('anaExportMsg');
  const need = () => { if(ANA_LAST) return true; cjhqFlashMsg(msg, 'Wait for the numbers to load first.', true); return false; };
  const pr = document.getElementById('anaPrint');
  if(pr) pr.addEventListener('click', () => {
    if(!need()) return;
    document.body.classList.add('ana-printing');
    const done = () => { document.body.classList.remove('ana-printing'); window.removeEventListener('afterprint', done); };
    window.addEventListener('afterprint', done);
    setTimeout(() => { window.print(); setTimeout(done, 1500); }, 50);
  });
  const csv = document.getElementById('anaCsv');
  if(csv) csv.addEventListener('click', () => { if(need()) anaDownload(anaFileName(), anaCsv()); });
  const sh = document.getElementById('anaShare');
  if(sh) sh.addEventListener('click', async () => {
    if(!need()) return;
    const text = anaSummaryText();
    try{
      const file = new File([anaCsv()], anaFileName(), { type:'text/csv' });
      if(navigator.canShare && navigator.canShare({ files:[file] })){ await navigator.share({ title:'CJHQ website analytics', text, files:[file] }); return; }
      if(navigator.share){ await navigator.share({ title:'CJHQ website analytics', text }); return; }
    }catch(e){ if(e && e.name === 'AbortError') return; }
    const ok = await cjhqCopyText(text);
    anaDownload(anaFileName(), anaCsv());
    cjhqFlashMsg(msg, ok ? 'Summary copied and spreadsheet downloaded - paste the summary and attach the file.' : 'Spreadsheet downloaded.', false);
  });
}

/* ---------- Tracking links (?r=<label>) ---------- */
function anaTrkUrl(l){ return 'https://cjhq.org' + (l.path === '/' ? '/' : l.path) + '?r=' + encodeURIComponent(l.label); }
function anaRenderTrackingList(){
  const box = document.getElementById('trkList'); if(!box) return;
  const esc = cjhqEscapeHtml;
  const g = (ANA_LAST && ANA_LAST.ga) || {};
  const dl = document.getElementById('trkPageList');
  if(dl){
    const paths = new Set(['/']);
    (g.pages || []).forEach(p => { if(p.path && !/^\/admin/.test(p.path)) paths.add(p.path); });
    ANA_LINKS.forEach(l => paths.add(l.path));
    dl.innerHTML = [...paths].slice(0, 80).map(p => `<option value="${esc(p)}">`).join('');
  }
  const counts = {};
  (g.tracked || []).forEach(x => { counts[x.label + '|' + x.page] = x; });
  if(!ANA_LINKS.length){ box.innerHTML = '<p class="adm-muted" style="font-size:.84rem;">No tracking links yet.</p>'; return; }
  box.innerHTML = ANA_LINKS.map((l, i) => {
    const c = counts[l.label + '|' + l.path];
    return `<div class="ana-trk-row"><b>${esc(l.label)}</b><code>${esc(anaTrkUrl(l))}</code>`
      + `<span class="adm-muted">${c ? anaFmtNum(c.visits) + ' visits' : '0 visits'} (${esc(g.label || 'selected period')})</span>`
      + `<button class="admin-small-btn" type="button" data-trk="copy" data-i="${i}">Copy</button>`
      + `<button class="admin-small-btn" type="button" data-trk="wa" data-i="${i}">WhatsApp</button>`
      + `<button class="admin-small-btn" type="button" data-trk="del" data-i="${i}">Remove</button></div>`;
  }).join('');
}
async function anaLinksCall(body){
  const token = await adminUser.getIdToken();
  const r = await fetch(ANALYTICS_URL + (body ? '' : '?links=1'), body
    ? { method:'POST', headers:{ authorization:'Bearer ' + token, 'content-type':'application/json' }, body: JSON.stringify(body) }
    : { headers:{ authorization:'Bearer ' + token } });
  const data = await r.json().catch(() => ({}));
  if(!r.ok) throw new Error(data.error || ('HTTP ' + r.status));
  ANA_LINKS = data.links || [];
  anaRenderTrackingList();
}
function anaWireTrackingLinks(){
  const msg = document.getElementById('trkMsg');
  const make = document.getElementById('trkMake');
  if(make) make.addEventListener('click', async () => {
    let path = ((document.getElementById('trkPage')||{}).value || '').trim();
    const label = ((document.getElementById('trkLabel')||{}).value || '').trim().toLowerCase().replace(/\s+/g, '-');
    path = path.replace(/^https?:\/\/(www\.)?cjhq\.org/i, '').split(/[?#]/)[0] || '/';
    if(!path.startsWith('/')) path = '/' + path;
    if(path.length > 1) path = path.replace(/\/+$/, '');
    if(!/^[a-z0-9][a-z0-9_-]{0,39}$/.test(label)){ cjhqFlashMsg(msg, 'Name: letters, numbers, - or _ only (e.g. facebook, yossi).', true); return; }
    try{
      await anaLinksCall({ op:'add', label, path });
      const ok = await cjhqCopyText(anaTrkUrl({ label, path }));
      cjhqFlashMsg(msg, ok ? 'Link made and copied.' : 'Link made.', false);
      const lb = document.getElementById('trkLabel'); if(lb) lb.value = '';
    }catch(e){ cjhqFlashMsg(msg, 'Could not save: ' + ((e && e.message) || e), true); }
  });
  const box = document.getElementById('trkList');
  if(box) box.addEventListener('click', async ev => {
    const b = ev.target.closest('button[data-trk]'); if(!b) return;
    const l = ANA_LINKS[Number(b.dataset.i)]; if(!l) return;
    if(b.dataset.trk === 'copy'){ const ok = await cjhqCopyText(anaTrkUrl(l)); cjhqFlashMsg(msg, ok ? 'Link copied' : 'Could not copy', !ok); }
    else if(b.dataset.trk === 'wa'){ window.open('https://wa.me/?text=' + encodeURIComponent(anaTrkUrl(l)), '_blank', 'noopener'); }
    else if(b.dataset.trk === 'del'){
      if(!(await admConfirm('Remove the "' + l.label + '" link from this list? Visits already counted stay in Google Analytics.'))) return;
      try{ await anaLinksCall({ op:'delete', label:l.label, path:l.path }); }catch(e){ cjhqFlashMsg(msg, 'Could not remove: ' + ((e && e.message) || e), true); }
    }
  });
  if(adminUser) anaLinksCall(null).catch(() => {});
}
...
async function renderAnalytics(){
  ...
  if(!body) return;
  const custom = document.getElementById('anaCustom');
  if(btn && !btn.dataset.wired){
    ...
    if(sel) sel.addEventListener('change', ()=>{
      ...
      if(isCustom){
        const s = document.getElementById('anaStart'), e = document.getElementById('anaEnd');
        const today = new Date().toLocaleDateString('en-CA', { timeZone:'America/Toronto' });
        if(e && !e.value) e.value = today;
        if(s && !s.value) s.value = admAddDaysISO(today, -6);
        if(e) e.max = today; if(s) s.max = today;
        return;
      }
      renderAnalytics();
    });
    const apply = document.getElementById('anaApply');
    if(apply) apply.addEventListener('click', renderAnalytics);
    anaWireExports();
    anaWireTrackingLinks();
  }
  if(!adminUser){ body.innerHTML = '<p class="adm-muted">Sign in to see analytics.</p>'; return; }
  if(ANALYTICS_BUSY) return;
  const range = sel ? sel.value : '28';
  let qs = '?range=' + encodeURIComponent(range);
  if(range === 'custom'){
    const a = (document.getElementById('anaStart')||{}).value || '', b = (document.getElementById('anaEnd')||{}).value || '';
    if(!a || !b || a > b){ if(status) status.textContent = 'Pick a start date on or before the end date.'; return; }
    qs += '&start=' + encodeURIComponent(a) + '&end=' + encodeURIComponent(b);
  }
  ANALYTICS_BUSY = true;
  ...
  try{
    const token = await adminUser.getIdToken();
    const r = await fetch(ANALYTICS_URL + qs, { headers: { authorization: 'Bearer ' + token } });
    const data = await r.json().catch(() => ({}));
    if(!r.ok) throw new Error(data.error || ('HTTP ' + r.status));
    ...
    html += '<div class="ana-kpis">'
      ...
      + '</div>';
    html += '<section class="adm-dash" style="display:block; margin-bottom:18px;"><section><h3>' + (g.hourly ? 'Visitors per hour' : 'Visitors per day') + '</h3><div class="ana-chart">'
      + g.daily.map(d => `<div title="${esc(anaFmtBucket(d.date))}: ${d.visitors} visitors, ${d.pageViews} views" style="height:${Math.round(d.visitors / maxDay * 100)}%"></div>`).join('')
      + '</div></section></section>';
    ...
    if(dm && !dm.error){
      const locRows = (dm.cities && dm.cities.length) || (dm.countries && dm.countries.length);
      if(locRows){
        html += '<div class="adm-dash" style="margin-bottom:18px;">';
        html += '<section><h3>Where visitors are (top cities)</h3><table class="ana-table"><tr><th>Place</th><th style="text-align:right">Visitors</th></tr>'
          + rows((dm.cities || []).slice(0, 15), [
              { f: x => esc(x.place) },
              { cls:'n', f: x => anaFmtNum(x.visitors) },
            ]) + '</table></section>';
        html += '<section><h3>Top countries</h3><table class="ana-table"><tr><th>Country</th><th style="text-align:right">Visitors</th></tr>'
          + rows((dm.countries || []).slice(0, 15), [
              { f: x => esc(x.country) },
              { cls:'n', f: x => anaFmtNum(x.visitors) },
            ]) + '</table></section>';
        html += '</div>';
      }
      const hasAges = dm.ages && dm.ages.length, hasGenders = dm.genders && dm.genders.length;
      if(hasAges || hasGenders){
        html += '<div class="adm-dash" style="margin-bottom:18px;">';
        if(hasAges) html += '<section><h3>Age</h3><table class="ana-table"><tr><th>Age group</th><th style="text-align:right">Visitors</th></tr>'
          + rows(dm.ages, [
              { f: x => esc(x.bracket) },
              { cls:'n', f: x => anaFmtNum(x.visitors) },
            ]) + '</table></section>';
        if(hasGenders) html += '<section><h3>Gender</h3><table class="ana-table"><tr><th>Gender</th><th style="text-align:right">Visitors</th></tr>'
          + rows(dm.genders, [
              { f: x => esc(x.gender) },
              { cls:'n', f: x => anaFmtNum(x.visitors) },
            ]) + '</table></section>';
    ...
    html += '<section class="adm-dash" style="display:block; margin-bottom:18px;"><section><h3>Visits by tracking link</h3><table class="ana-table"><tr><th>Name / platform</th><th>Page</th><th style="text-align:right">Visits</th><th style="text-align:right">Visitors</th></tr>'
      + ((g.tracked && g.tracked.length) ? rows(g.tracked, [
          { f: x => esc(x.label) },
          { cls:'p', f: x => esc(x.page) },
          { cls:'n', f: x => anaFmtNum(x.visits) },
          { cls:'n', f: x => anaFmtNum(x.visitors) },
        ]) : '<tr><td colspan="4" class="adm-muted">No visits through tracking links in this period yet. Make links below.</td></tr>')
      + '</table></section></section>';
    html += '<div class="adm-dash" style="margin-bottom:18px;">';
